...
if uploaded_file is not None:
    ...
    with col2:
        ...
        # Show result based on confidence
        if confidence >= 0.8:
            st.success(f"Prediction: {disease_name}")
            st.write(f"Confidence: {confidence*100:.2f}% (High)")
        elif confidence >= 0.6:
            st.info(f"Prediction: {disease_name}")
            st.write(f"Confidence: {confidence*100:.2f}% (Medium)")
        else:
            st.warning(f"Prediction: {disease_name}")
            st.write(f"Confidence: {confidence*100:.2f}% (Low)")
