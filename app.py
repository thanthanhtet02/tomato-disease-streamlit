...
def load_model():
    ...
    # - Use weights=None here (we will load your trained weights from .h5)
    ...
    model = models.Sequential([
        ...
        # data_augmentation layer is NOT needed at inference (and has no weights)
        preprocess,
        base_model,
...
def preprocess_image(img: Image.Image):
    # IMPORTANT: keep pixels in 0..255 because preprocess_input expects that
    img = img.convert("RGB").resize((128, 128))
    arr = np.array(img).astype(np.float32)  # NO /255.0
